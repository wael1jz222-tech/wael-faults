...
const CAR_BRANDS = ["تويوتا","نيسان","هيونداي","كيا","فورد","جي إم سي","لكزس","مرسيدس","بي إم دبليو","أودي","شيفروليه","ميتسوبيشي","هوندا","دودج","جيب","رام","كرايسلر","إنفينيتي","أكورا","سوبارو","مازدا","فولكس واجن","بورشه","لاند روفر","فولفو","جينيسيس"];
const ACCENT = "#1d6fdb";
...
export default function App() {
  ...
  if (!session) return (
    <AuthScreen view={authView} setView={setAuthView} onLogin={handleLogin} onSignup={handleSignup} />
  );
  ...
  return (
    <MainApp session={session} profile={profile} isAdmin={isAdmin} canAdd={canAdd} onLogout={handleLogout} />
  );
}
...
function AuthScreen({ view, setView, onLogin, onSignup }) {
  ...
  async function handle() {
    ...
    try {
      ...
      else { await onSignup(email, password); setMsg("تم إنشاء الحساب!"); }
    ...
    <div style={{ fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", direction:"rtl", background:"#f5f7fa", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:32, width:"100%", maxWidth:380, boxShadow:"0 4px 24px #0002" }}>
        ...
        <div style={{ display:"flex", background:"#f5f7fa", borderRadius:10, padding:4, marginBottom:24 }}>
          <button onClick={()=>{setView("login");setError("");setMsg("");}} style={{ flex:1, background:view==="login"?"#fff":"none", border:"none", borderRadius:8, padding:"9px", fontWeight:700, fontSize:14, cursor:"pointer", color:view==="login"?ACCENT:"#888" }}>تسجيل دخول</button>
          <button onClick={()=>{setView("signup");setError("");setMsg("");}} style={{ flex:1, background:view==="signup"?"#fff":"none", border:"none", borderRadius:8, padding:"9px", fontWeight:700, fontSize:14, cursor:"pointer", color:view==="signup"?ACCENT:"#888" }}>حساب جديد</button>
        </div>
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif", direction:"rtl", background:"#f5f7fa", minHeight:"100vh", color:"#1a1a2e" }}>
      ...
      <div style={{ maxWidth:740, margin:"0 auto", padding:"24px 16px" }}>
        {view==="list" && (
          <>
            <div style={{ position:"relative", marginBottom:20 }}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="ابحث بكود العطل أو السيارة..." style={{ width:"100%", boxSizing:"border-box", background:"#fff", border:"1.5px solid #e5e9f0", borderRadius:10, padding:"11px 44px 11px 16px", fontSize:14, outline:"none" }} />
            </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                {filtered.map(fault=>(
                  <div key={fault.id} onClick={()=>{ setSelected(fault); setView("detail"); }}
                    style={{ background:"#fff", border:"1.5px solid #e5e9f0", borderRadius:14, padding:"16px 18px", cursor:"pointer" }}>
        ...
        {view==="add" && canAdd && (
          <div>
            <h2 style={{ margin:"0 0 22px", fontSize:18, fontWeight:800 }}>➕ إضافة عطل جديد</h2>
            <Sec title="كود العطل">
            ...
            <Sec title="صور وفيديوات 📸🎬">
              <div onDrop={handleMedia} onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}
                ...
                style={{ border:`2px dashed ${dragOver?ACCENT:"#d0d7e3"}`, borderRadius:12, padding:"28px", textAlign:"center", cursor:"pointer", background:dragOver?ACCENT_LIGHT:"#fafbfd" }}>
                <div style={{ fontSize:32, marginBottom:8 }}>📎</div>
                <div style={{ color:"#888", fontSize:13 }}>اسحب الصور والفيديوات هنا أو اضغط للرفع</div>
                <input id="mi" type="file" multiple accept="image/*,video/*" capture="environment" style={{ display:"none" }} onChange={handleMedia} />
              </div>
              {form.media.length>0 && (
